...
router.get("/filteredHuaweiProjectsNamesArray", (req, res) => {
  Row.find((err, vendorProjects) => {
    ...
    try {
      const projects = vendorProjects.filter(
        (obj) => obj.values[7] === "Huawei"
      );
...
router.get("/vendorProjectsDatabases", async (req, res, next) => {
  ...
  Row.find({ "values.7": "Huawei" }).exec((err, posts) => {
    if (err) {
      return res.status(400).json({
        error: err,
      });
...
function getProjectsScopeDataCount(posts, projectName) {
  var ScopeDataCount = [];
  if (projectName === "All Huawei Projects") {
...
router.get(
  "/vendorProjectsDatabasesChartDataColumnChartData",
  async (req, res, next) => {
    let ProjectName = req.query.Project; // Other Project 2023 or Other Project 2022 or All Huawei Projects

    let query = { "values.7": "Huawei" };
...
router.get(
  "/HuaweivendorProjectsDatabasesCompletion",
  async (req, res, next) => {
...
router.get("/huaweiProjectsLastUpdates", async (req, res, next) => {
  let ProjectName = req.query.Project;
